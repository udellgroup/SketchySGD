...
class Logistic(GLM):
    ...
    def get_losses(self, v = None, both = True):
        """Get the train and test losses.

        Args:
            v (ndarray, optional): Location at which we should evaluate losses. Defaults to None.
        ...
        if self.fit_intercept: # Don't incorporate bias into regularization
            train_loss = 1/self.ntr * sum(np.log(1 + np.exp(-np.multiply(self.btr, self.Atr @ v)))) + self.mu/2 * np.linalg.norm(v[1:])**2
        else:
            train_loss = 1/self.ntr * sum(np.log(1 + np.exp(-np.multiply(self.btr, self.Atr @ v)))) + self.mu/2 * np.linalg.norm(v)**2 

        if both:
            test_loss = 1/self.ntst * sum(np.log(1 + np.exp(-np.multiply(self.btst, self.Atst @ v))))
            return {'train_loss': train_loss, 'test_loss': test_loss}
        else:
            return {'train_loss': train_loss}

    def get_acc(self, v = None):
        """Get the train and test accuracies. Useful for classification problems w/ least squares.

        Args:
            v (ndarray, optional): Location at which we should evaluate accuracies. Defaults to None.
