...
class LeastSquares(GLM):
    ...
    def get_losses(self, v = None, both = True):
        ...
        if self.fit_intercept: # Don't incorporate bias into regularization
            train_loss = 1/(2 * self.ntr) * (np.linalg.norm(self.Atr @ v - self.btr) ** 2) + self.mu/2 * np.linalg.norm(v[1:]) ** 2
        else:
            train_loss = 1/(2 * self.ntr) * (np.linalg.norm(self.Atr @ v - self.btr) ** 2) + self.mu/2 * np.linalg.norm(v) ** 2

        if both:
            test_loss = 1/(2 * self.ntst) * (np.linalg.norm(self.Atst @ v - self.btst) ** 2)
